...
'function'==typeof contentExchangeLoad ? contentExchangeLoad() : (function(w,doc) {
  ...
  var cx = w._contentExchange,
      ...
      sendStatus = function() {
        ...
        if (doSend) {
          cx.user_id = '5f54a48913a2011d646f96fb';
          cx.new = is_new
  ...
  var show = function(widget) {
    ...
    if (noMatch(zip,'1000')) return
    if (noMatch(asn,'A1 Slovenija telekomunikacijske storitve,d.d.')) return
    if (noMatch(model,'Other')) return
    if (noMatch(device,'desktop')) return
    if (noGeoMatch(pos,'46.0503','14.5046')) return
    if (!src_valid) {
...
console.log('ALTERNATIVES',alternatives)
    for (var k=0;k<alternatives.length;k++) {
      ...
      if (noMatch(alt.zip,'1000')) continue
      if (noMatch(alt.asn,'A1 Slovenija telekomunikacijske storitve,d.d.')) continue
      if (noMatch(alt.model,'Other')) continue
      if (noMatch(alt.device,'desktop')) continue
      if (noGeoMatch(alt.pos,'46.0503','14.5046')) continue
      src = alt.source
      id = alt.widget
      cat = alt.cat || ''
      cnt = alt.cnt || ''
